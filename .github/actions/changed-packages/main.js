...
execute(async () => {
  const files = JSON.parse(core.getInput('files', '[]')).map((f) => path.resolve(root, f));
  const packages = await ls();
  const changedPackages = packages.filter((pkg) => files.some((f) => f.startsWith(pkg.location)));
  const paths = JSON.stringify(changedPackages.map((pkg) => path.relative(root, pkg.location)));
